            ...
            <form action="{{ route('payment.payment_systems.update',['payment_system'   => $payment_system->id]) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="exampleFormControlSelect1" name="status">
                        <option>{{ \Teamprodev\LaravelPayment\Models\PaymentSystem::ACTIVE }}</option>
                        <option @if($payment_system->status == \Teamprodev\LaravelPayment\Models\PaymentSystem::NOT_ACTIVE) selected @endif>{{ \Teamprodev\LaravelPayment\Models\PaymentSystem::NOT_ACTIVE }}</option>
                    </select>
                </div>
                <div class="col-12" id="fieldsList">
                    {!! \Teamprodev\LaravelPayment\Services\PaymentSystemParamService::render($payment_system->system) !!}
                </div>
                <div class="col-12">
